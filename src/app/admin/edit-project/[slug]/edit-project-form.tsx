...
// src/app/admin/edit-project/[slug]/edit-project-form.tsx
'use client';
...
export function EditProjectForm({ project }: { project: Project }) {
  ...
  const onSubmit = async (values: FormData) => {
    try {
      ...
      const result = await updateProjectAction(project.slug, projectData);
      
      toast({
        title: 'Projet mis à jour !',
        description: `Le projet "${result.title}" a été modifié avec succès.`,
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Impossible de modifier le projet.',
      });
    }
  };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex gap-4">
            ...
            >
                ...
                {form.formState.isSubmitting ? 'Sauvegarde...' : 'Mettre à jour le projet'}
            </Button>
             <Button type="button" variant="ghost" onClick={() => router.back()} disabled={form.formState.isSubmitting}>
                Annuler
            </Button>
        </div>
